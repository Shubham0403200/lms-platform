...
const Blogs:React.FC<BlogsProps> = ({ blogs, loading }) => {
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="text-center mb-8">
          <h2 className="text-h1-clamp font-extrabold text-slate-800 mb-2">
            101 <span className="text-blue-600">Live blogs</span>
          </h2>
            ...
            : blogs.slice(0, 3).map((event, i) => (
                ...
                >
                  <Link href={`/blogs/${event.slug}`}>
                    <div className="rounded-xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-md transition">
                      <div className="h-[180px] md:h-[220px] overflow-hidden relative">
                        <img
                          src={(event as any)?.thumbnail?.secure_url || "/images/blogs/fallback.jpg"}
                          alt={event.title}
                      ...
                      <div className="p-3 md:p-4 space-y-2">
                        ...
                        <div className="flex items-center gap-2 text-h7-clamp text-blue-600 font-medium mt-2">
                          <span className="line-clamp-1">{event.writer} •{" "}
                        {event.createdAt &&
                          new Date(event.createdAt).toLocaleDateString("en-IN")} </span>
                        </div>
                      </div>
                    </div>
                  </Link>
